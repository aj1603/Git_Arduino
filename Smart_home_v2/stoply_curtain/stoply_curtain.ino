...
IPAddress staticIP(192, 168, 1, 23); //ESP static ip
...
void setup(void){
  ...
  server.on("/control/1", handle0on); //as Per  <a href="ledOn">, Subroutine to be called
  server.on("/control/0", handle0off);
  server.on("/control/2", handle0offff);
